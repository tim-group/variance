...
public class RegularTypeConverterDictionary implements TypeConverterDictionary {
    ...
    public RegularTypeConverterDictionary() {
        registry = Maps.newHashMap();
    }
    
    public RegularTypeConverterDictionary(Map<Class<?>, Map<Class<?>, Function<?, ?>>> registry) {
        this.registry = registry;
    }
    
    @Override public Set<Class<?>> sourceClasses() {
        return registry.keySet();
    }

    @Override public Set<Class<?>> targetClassesFor(Class<?> sourceClass) {
        if (registry.containsKey(sourceClass)) {
    ...
    @Override public <S, T> Function<S, T> converterFor(Class<S> sourceClass, Class<T> targetClass) {
        if (!registry.containsKey(sourceClass)) {
            return null;
        }
